...
def _do_3d_plotting(pdb_file, coords, labels, view=None, color='', bond_color=None, alpha=1.0, show_snfg=True,
    ...
    pdb_content = Path(pdb_file).read_text() if isinstance(pdb_file, (Path, str)) else df_to_pdb_content(pdb_file[0])
    if pos != 'ref':
    ...
    for i, (coord, label) in enumerate(zip(coords, labels)):
        ...
        mono_id, mono_name = label.split('_')[:2]
...
def find_difference(glycans, pattern=None, alternative=None, metric="SASA", struc_dict=structure_graphs, plot=False):
    ...
    for with_pat, without_pat in twins:
        ...
        if not (graph_with and graph_without):
            continue
        with_pat_value = calculate_average_metric(graph_with, pattern, metric)
    ...
    if not with_pattern_values:
        return {"pattern": pattern, "alternative": alternative, "metric": metric, "n_pairs": 0, "twins": twins}
    ...
    if plot:
        ...
        rng = np.random.default_rng(42)  # For consistent jitter
        for i, (label, values) in enumerate(data.items()):
            noise = rng.normal(0, 0.04, size = len(values))
...
def ramachandran_plot(disaccharide: str,
    ...
    # Add statistics
    phi_rad, psi_rad = np.radians(phi_angles), np.radians(psi_angles)
    phi_r = min(np.hypot(np.mean(np.sin(phi_rad)), np.mean(np.cos(phi_rad))), 1.0)
    psi_r = min(np.hypot(np.mean(np.sin(psi_rad)), np.mean(np.cos(psi_rad))), 1.0)
    stats_text = (
        f"n = {len(phi_angles)}\n"
        f"Mean φ = {np.degrees(np.arctan2(np.mean(np.sin(phi_rad)), np.mean(np.cos(phi_rad)))):.1f}° ± {np.degrees(np.sqrt(-2 * np.log(phi_r))):.1f}°\n"
        f"Mean ψ = {np.degrees(np.arctan2(np.mean(np.sin(psi_rad)), np.mean(np.cos(psi_rad)))):.1f}° ± {np.degrees(np.sqrt(-2 * np.log(psi_r))):.1f}°"
    )
